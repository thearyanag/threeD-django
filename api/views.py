...
from api.serializers import CategorySerializer,ThreeDModelSerializer
...
def threeDModelApi(request,id=0):
    if request.method == "GET":
        threeDModel = ThreeDModel.objects.all()
        threeD_serializer = ThreeDModelSerializer(threeDModel , many=True)
        return JsonResponse(threeD_serializer.data,safe=False)
    elif request.method == "POST":
        threeD_data = JSONParser().parse(request)
        threeD_serializer = ThreeDModelSerializer(data=threeD_data)
        ...
        threeD_data = JSONParser().parse(request)
        threeDModel = ThreeDModel.objects.get(ThreeDId = threeD_data["ThreeDId"])
        threeD_serializer = ThreeDModelSerializer(threeDModel , data=threeD_data)
        if threeD_serializer.is_valid():
            threeD_serializer.save()
            return JsonResponse("Updated successfully" , safe=False)
        return JsonResponse("Faild to update" , safe=False)
    elif request.method == "DELETE":
        threeDModel = ThreeDModel.objects.get(ThreeDId = id)
        threeDModel.delete()
        return JsonResponse("Deleted Successfully" , safe=False)
